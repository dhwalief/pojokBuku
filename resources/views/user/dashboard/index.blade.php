                            ...
                            <div class="flex items-start space-x-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                <div class="flex-shrink-0">
                                    @if($borrow->book->url_cover)
                                        <img src="{{ $borrow->book->url_cover }}" alt="{{ $borrow->book->title }}" class="w-16 h-20 object-cover rounded-lg">
                                    @else
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="mt-2 flex items-center space-x-4 text-xs text-gray-500 dark:text-gray-400">
                                        <span>Dipinjam: {{ $borrow->date_borrowed->format('d M Y') }}</span>
                                        <span>•</span>
                                        <span>Batas: {{ $borrow->date_returned->format('d M Y') }}</span>
                                    </div>
                                    @php
                                        $daysLeft = now()->diffInDays($borrow->date_returned, false);
                                    @endphp
                                    @if($daysLeft > 0)
                    ...
                    @if($recentBorrows && $recentBorrows->count() > 0)
                        <div class="space-y-4">
                            ...
                            <div class="flex items-start space-x-4">
                                <div class="flex-shrink-0">
                                    @if($borrow->book->url_cover)
                                        <img src="{{ $borrow->book->url_cover }}" alt="{{ $borrow->book->title }}" class="w-12 h-16 object-cover rounded-lg">
                                    @else
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="mt-1 flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
                                        <span>{{ $borrow->date_borrowed->format('d M Y') }} - {{ $borrow->date_returned->format('d M Y') }}</span>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <a href="{{ route('books.index') }}" class="flex items-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors">
                        ...
                        <div class="ml-3">
                            <p class="text-sm font-medium text-blue-900 dark:text-blue-300">Cari Buku</p>
                            <p class="text-xs text-blue-700 dark:text-blue-400">Temukan buku favorit</p>
                        </div>
                    </a>
                    ...
                    <a href="{{ route('categories.index') }}" class="flex items-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors">
                        ...
                        <div class="ml-3">
                            <p class="text-sm font-medium text-green-900 dark:text-green-300">Kategori</p>
                            <p class="text-xs text-green-700 dark:text-green-400">Jelajahi berdasarkan genre</p>
                        </div>
                    </a>
                    ...
                    <a href="{{ route('borrows.index') }}" class="flex items-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors">
                        ...
                        <div class="ml-3">
                            <p class="text-sm font-medium text-purple-900 dark:text-purple-300">Riwayat</p>
                            <p class="text-xs text-purple-700 dark:text-purple-400">Lihat semua peminjaman</p>
                        </div>
                    </a>
                    ...
                    <a href="{{ route('profile.edit') }}" class="flex items-center p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg hover:bg-orange-100 dark:hover:bg-orange-900/30 transition-colors">
                        ...
                        <div class="ml-3">
                            <p class="text-sm font-medium text-orange-900 dark:text-orange-300">Profil</p>
                            <p class="text-xs text-orange-700 dark:text-orange-400">Edit informasi akun</p>
                        </div>
                    </a>
                </div>
            </div>
